...
const TWO_FACTOR_ERROR = "Error.InvalidTOTPAndCode";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!res.ok) {
      // Phát hiện lỗi 2FA: backend trả 422 kèm Error.InvalidTOTPAndCode
      const messages: { message: string }[] = Array.isArray(data?.message) ? data.message : [];
      const isTwoFactorRequired = messages.some((m) => m.message === TWO_FACTOR_ERROR);
